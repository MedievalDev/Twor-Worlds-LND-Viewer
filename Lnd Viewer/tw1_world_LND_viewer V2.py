...
def parse_lnd_maps(filepath):
    ...
    for _ in range(cnt):
        ...
        tex_names.append(data[pos:pos+sl].decode("ascii", errors="replace"))
        pos += sl
    m["tex_names"] = tex_names
    pos += 1  # unk byte
    
    # Tex Ref Map (32x32, 4 texture indices per cell)
    w, pos = _u32(data, pos); h, pos = _u32(data, pos)
    m["tex_ref"] = {"w": w, "h": h, "data": data[pos:pos+w*h*4]}; pos += w*h*4
    # Tex Alpha Map (544x544, 4 blend weights per pixel)
    w, pos = _u32(data, pos); h, pos = _u32(data, pos)
    m["tex_alpha"] = {"w": w, "h": h, "data": data[pos:pos+w*h*4]}; pos += w*h*4
...
# ============================================================
# CHANNEL-PACKED SPLATMAP EXPORT
# ============================================================
def export_channel_packed_splatmaps(tiles, out_dir):
    """Export channel-packed terrain splatmaps for each tile.
    
    For each tile: reads tex_ref (32x32, 4 indices) + tex_alpha (544x544, 4 weights),
    generates per-texture grayscale masks, then packs 4 masks into RGBA channels
    of a single PNG. Max 16 textures = max 4 PNGs per tile.
    Also writes a CSV manifest mapping channels to texture names.
    """
    os.makedirs(out_dir, exist_ok=True)
    exported = []
    ...
    for (col, row), m in sorted(tiles.items()):
        tr = m.get("tex_ref")
        ta = m.get("tex_alpha")
        tnames = m.get("tex_names", [])
        if not tr or not ta or tr["w"] == 0 or ta["w"] == 0:
            continue
        
        letter = col_to_letter(col)
        num = f"{row+1:02d}"
        tile_id = f"{letter}{num}"
        
        ref_arr = np.frombuffer(tr["data"], dtype=np.uint8).reshape(tr["h"], tr["w"], 4)
        alpha_arr = np.frombuffer(ta["data"], dtype=np.uint8).reshape(ta["h"], ta["w"], 4)
        
        ah, aw = ta["h"], ta["w"]
        rh, rw = tr["h"], tr["w"]
        cell_h = ah // rh  # typically 17
        cell_w = aw // rw
        
        # Find unique texture IDs (exclude 255 = empty)
        unique_ids = sorted([int(x) for x in np.unique(ref_arr) if x != 255])
        if not unique_ids:
            continue
        
        # Build per-texture splatmaps
        splatmaps = {tid: np.zeros((ah, aw), dtype=np.uint8) for tid in unique_ids}
        
        for ry in range(rh):
            for rx in range(rw):
                cell_tex = ref_arr[ry, rx]
                ay0 = ry * cell_h
                ay1 = min(ay0 + cell_h, ah)
                ax0 = rx * cell_w
                ax1 = min(ax0 + cell_w, aw)
                
                for ch in range(4):
                    tid = int(cell_tex[ch])
                    if tid == 255:
                        continue
                    region = alpha_arr[ay0:ay1, ax0:ax1, ch]
                    splatmaps[tid][ay0:ay1, ax0:ax1] = np.maximum(
                        splatmaps[tid][ay0:ay1, ax0:ax1], region
                    )
        
        # Channel-pack: 4 textures per RGBA PNG
        manifest_lines = [f"tile,png,channel,tex_index,tex_name"]
        channels = ["R", "G", "B", "A"]
        
        for i in range(0, len(unique_ids), 4):
            group = unique_ids[i:i+4]
            rgba = np.zeros((ah, aw, 4), dtype=np.uint8)
            
            for ch, tid in enumerate(group):
                rgba[:, :, ch] = splatmaps[tid]
                tname = tnames[tid] if tid < len(tnames) else f"tex_{tid}"
                manifest_lines.append(f"{tile_id},splatmap_{i//4},{channels[ch]},{tid},{tname}")
            
            # Flip vertically (LND stores bottom-to-top)
            rgba = np.flipud(rgba)
            
            fname = f"Map_{tile_id}_splatmap_{i//4}.png"
            fpath = os.path.join(out_dir, fname)
            Image.fromarray(rgba, mode="RGBA").save(fpath)
            exported.append(fpath)
        
        # Write manifest CSV for this tile
        csv_path = os.path.join(out_dir, f"Map_{tile_id}_splatmap_manifest.csv")
...
class App:
    ...
    def _build_ui(self):
        ...
        tk.Button(tb, text="Export Splatmaps", bg=ORANGE, fg="#000",
                  font=("Segoe UI", 10, "bold"), bd=0, padx=10,
                  command=self._export_splatmaps).pack(side="right", padx=4)
    ...
    def _export_splatmaps(self):
        if not self.current_tiles: return
        folder = filedialog.askdirectory(title="Export channel-packed splatmaps to folder")
        if not folder: return
        self.status_lbl.config(text="Exporting splatmaps (channel-packed)..."); self.root.update()
        exported = export_channel_packed_splatmaps(self.current_tiles, folder)
        png_count = sum(1 for f in exported if f.endswith(".png"))
        csv_count = sum(1 for f in exported if f.endswith(".csv"))
        self.status_lbl.config(text=f"Exported {png_count} splatmap PNGs + {csv_count} manifests")
